(() => {
  ...
  const pens = [
    ...
    // Pen 4 – Fireworks
    {
      ...
      mount(mountEl) {
        ...
        this._soundManager = soundManager4;
      ...
      unmount() {
        if (this._soundManager) {
          try { this._soundManager.ctx.close(); } catch(e) {}
  ...
  // ─────────────────────────────────────────────────────────
  // NAV UI — comment out the next line to disable
  var SHOW_NAV_UI = true;
  // ─────────────────────────────────────────────────────────

  var currentIndex = Math.floor(Math.random() * pens.length);
  var activePen = null;
  var navUI = null;

  function createNavUI() {
    var ui = document.createElement("div");
    ui.id = "iprocess-pen-nav";
    ui.style.cssText = [
      "position:absolute",
      "bottom:24px",
      "left:50%",
    ...
    prev.textContent = "‹";
    prev.title = "Previous animation";
    prev.style.cssText = btnStyle;

    var label = document.createElement("span");
    label.id = "iprocess-pen-label";
    label.style.cssText = "min-width:120px;text-align:center;color:#fff;font-weight:500;";

    var next = document.createElement("button");
    next.textContent = "›";
    next.title = "Next animation";
    next.style.cssText = btnStyle;

    prev.addEventListener("mouseenter", function() { this.style.background = "rgba(208,168,92,0.18)"; });
    prev.addEventListener("mouseleave", function() { this.style.background = "none"; });
    next.addEventListener("mouseenter", function() { this.style.background = "rgba(208,168,92,0.18)"; });
    next.addEventListener("mouseleave", function() { this.style.background = "none"; });

    prev.addEventListener("click", function() {
      currentIndex = (currentIndex - 1 + pens.length) % pens.length;
      mountPen();
    });

    next.addEventListener("click", function() {
      currentIndex = (currentIndex + 1) % pens.length;
      mountPen();
    });
  ...
  function updateLabel() {
    var label = document.getElementById("iprocess-pen-label");
    if (label) {
      label.textContent = pens[currentIndex].welcomeName + " – " + pens[currentIndex].name;
    }
  }

  function getNavContainer() {
    return document.querySelector(".intro-overlay") || document.body;
  }

  function ensureNavUI() {
    if (!document.getElementById("iprocess-pen-nav")) {
      getNavContainer().appendChild(navUI);
    }
  }

  async function mountPen() {
    var stage = document.querySelector("#intro-stage, .intro-stage");
    if (!stage) return;
    if (activePen && typeof activePen.unmount === "function") activePen.unmount();
    ...
    if (SHOW_NAV_UI) updateLabel();
    stage.innerHTML = "";
    for (var i = 0; i < pen.deps.length; i++) {
      try { await loadScript(pen.deps[i]); } catch (e) { console.error("Failed to load", pen.deps[i], e); }
    }
    try { pen.mount(stage); } catch (e) { console.error("Pen error:", e); }
    if (SHOW_NAV_UI) ensureNavUI();
  }
